...
<%@include file="../common/menubar.jsp" %>
	...
	<section id="content-2">
		...
		<div id="login-section">
			<form method="post" id="login_wrap" action="../../login.me">
				<!-- 나중에 action 추가하기 -->
				<fieldset>
					...
					<div id="login_area" align="center">
						...
						<div class="error" id="error_id">
							<!-- <span id="error_id_msg"></span> -->
						</div>
						<!-- 아이디 입력 오류시  문자열 출력-->

						
						<input type="password" id="userPwd" name="userPwd"
							placeholder="비밀번호 입력" maxlength="40">
						<div class="error" id="error_pwd">
							<% if (msg != null) { %>
							
							<span id="fail_msg"><%=msg%></span>
...
<script>
		$("#login_wrap").submit(function() {
			if ($("#userId").val().trim().length == 0) {  // 아이디 입력값이 없을 때
				$("#error_id").text("아이디를 확인하세요.")
					.css({"color" : "red", "font-size" : "15px"});		
					// 아이디 input 창 아래 "아이디 입력하세요 "출력
				return false;
			}
	
			if ($("#userPwd").val().trim().length == 0) { // 비밀번호 입력값이 없을 때
				$("#error_pwd").text("비밀번호를 확인하세요.")
						.css({"color" : "red", "font-size" : "15px" });
